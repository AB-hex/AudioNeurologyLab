function varargout = SpeechOptions(varargin)
% SPEECHOPTIONS M-file for SpeechOptions.fig
%      SPEECHOPTIONS, by itself, creates a new SPEECHOPTIONS or raises the existing
%      singleton*.
%
%      H = SPEECHOPTIONS returns the handle to a new SPEECHOPTIONS or the handle to
%      the existing singleton*.
%
%      SPEECHOPTIONS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SPEECHOPTIONS.M with the given input arguments.
%
%      SPEECHOPTIONS('Property','Value',...) creates a new SPEECHOPTIONS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before SpeechOptions_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to SpeechOptions_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help SpeechOptions

% Last Modified by GUIDE v2.5 30-Nov-2022 18:19:39

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @SpeechOptions_OpeningFcn, ...
                   'gui_OutputFcn',  @SpeechOptions_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before SpeechOptions is made visible.
function SpeechOptions_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to SpeechOptions (see VARARGIN)
...
% Choose default command line output for SpeechOptions
...
% UIWAIT makes SpeechOptions wait for user response (see UIRESUME)
% uiwait(handles.PTOptionsBackground);


% --- Outputs from this function are returned to the command line.
function varargout = SpeechOptions_OutputFcn(hObject, eventdata, handles) 
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
 ...
 mdb.(TXNumber).stimulus.speech.amp = value;
...
load mdb;
sliderMax = get(mdb.(TXNumber).stimulus.speech.ampSelectSlider_handle,'Max');
sliderMin = get(mdb.(TXNumber).stimulus.speech.ampSelectSlider_handle,'Min');
...
mdb.(TXNumber).stimulus.speech.amp = value;
set(mdb.(TXNumber).stimulus.speech.ampSelectSlider_handle,'Value',value);
...
if(1==get(hObject,'Value'))
 UpdateProperties(propertiesObj,mdb.(TXNumber).stimulus.speech);
...
switch str{val}
    case '0 deg'
       mdb.(TXNumber).stimulus.speech.phase = 0;
    case '180 deg'
       mdb.(TXNumber).stimulus.speech.phase = 180;   
end
...
% --- Executes on button press in stimulusSpeechSourceSelect_pushButton.
function stimulusSpeechSourceSelect_pushButton_Callback(hObject, eventdata, handles)
% hObject    handle to stimulusSpeechSourceSelect_pushButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
      TXNumber = strcat('TX',get(handles.TXNumber,'String'));
      ...
      disp(fullfilename);
      load mdb;
      mdb.(TXNumber).stimulus.speech.source = fullfilename;
      set( handles.stimulusSpeechSourceSelect_pushButton,'String',FileName);
      save mdb mdb;
