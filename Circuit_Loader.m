...
%
%   Behavioral (RX8, sound) only — for EEG (RA16), connect directly via
%   actxcontrol('RPco.x',...) + ConnectRA16, see startupFcn/LiveEEGMonitor.m.
%   Never call this while OpenWorkbench is running — it clears the RX8 COF
%   and destroys any active Workbench recording session.
    ...
    if wbRunning
        warning(['Circuit_Loader: OpenWorkbench is open. Calling this will clear ' ...
                 'the RX8 COF and destroy the active Workbench session.']);
    end
